...
**Watch scope.** If the MongoDB URI omits a database path, v1 uses `mongoc_client_watch`, bootstraps every non-excluded database, ignores steady-state events for excluded databases, and requires privileges for a cluster-wide change stream through `admin`. If the URI includes a database path, v1 uses `mongoc_database_watch`, bootstraps only that explicit database, and treats that path as an operator-selected scope that bypasses the excluded-database list.
...
- `insert`, `update`, `replace`, `delete` — document data mutations; each produces a `scribe_change_event` (insert/update/replace → payload; delete → tombstone). Batched by transaction when applicable.
...
- `modify`, `createIndexes`, `dropIndexes`, `refineCollectionShardKey`, `reshardCollection`, `shardCollection` — collection/schema/sharding metadata. MongoDB's `modify` operationType represents collection metadata changes such as `collMod`; document updates use `update`. v1 logs and ignores these because the object model intentionally stores document contents only.
- `drop` — subtree-changing DDL. If MongoDB accepts the event resume token, v1 writes one commit containing a tombstone for `database/collection` and persists that event token after the commit lands. No MongoDB rescan is needed.
- `dropDatabase` — subtree-changing DDL. If MongoDB accepts the event resume token, v1 writes one commit containing a tombstone for `database`. If MongoDB later emits `invalidate` for the database-scoped stream, v1 handles that later event with full bootstrap. If the dropDatabase token itself is unusable, v1 falls back immediately.
- `rename` — subtree-changing DDL. If MongoDB accepts the event resume token, v1 writes one commit containing the old-subtree tombstone plus additions from scanning only the renamed target collection. The commit persists the DDL event token, not a post-scan token, so writes racing with the scan are replayed from the change stream. Cluster-scoped exclusions are applied to both source and target databases.
- `invalidate` and unusable DDL resume tokens — stream-boundary events. v1 marks the resume token invalid, runs full bootstrap again, writes a new baseline commit parented to existing history, persists the replacement token, and enters steady-state watch. Full bootstrap is O(watch scope), so a cluster-scoped watch rescans every non-excluded database and stalls ingestion during the scan.
