...
ce-createIndexes## Summary

`createIndexes`
A `createIndexes` event occurs when an index is created on the collection and the change stream has the [showExpandedEvents](https://mongodbcom-cdn.staging.corp.mongodb.com/docs/reference/change-events/#std-label-change-streams-expanded-events) option set to `true`.
...
<td headers="Description">
An array of documents listing the indexes that were created by the operation.
...
<td headers="Description">
Together with the [lsid](https://mongodbcom-cdn.staging.corp.mongodb.com/docs/reference/change-events/createIndexes/#std-label--idref--lsid), a number that helps uniquely identify a transction.
