...
<td headers="Description">
Together with the [lsid](https://mongodbcom-cdn.staging.corp.mongodb.com/docs/reference/change-events/drop/#std-label--idref--lsid), a number that helps uniquely identify a transction.
...
A `drop` event leads to an [`invalidate`](https://mongodbcom-cdn.staging.corp.mongodb.com/docs/reference/change-events/invalidate/#mongodb-data-invalidate) event for change streams opened against its own `ns` collection.
