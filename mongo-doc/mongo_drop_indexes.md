...
ce-dropIndexes## Synopsis

`dropIndexes`
A `dropIndexes` event occurs when an index is dropped from the collection and the change stream has the [showExpandedEvents](https://mongodbcom-cdn.staging.corp.mongodb.com/docs/reference/change-events/#std-label-change-streams-expanded-events) option set to `true`.
...
<td headers="Description">
An array of documents listing the indexes that were dropped by the operation.
...
<td headers="Description">
Together with the [lsid](https://mongodbcom-cdn.staging.corp.mongodb.com/docs/reference/change-events/dropIndexes/#std-label--idref--lsid), a number that helps uniquely identify a transction.
